    ...
    class NamedArgs < Array
      ...
      def to_formulae_and_casks(
        ...
        @to_formulae_and_casks[only] ||= begin
          ...
          options = { warn:, download_queue: }.compact
          formulae_and_casks = downcased_unique_named.flat_map do |name|
            load_and_fetch_full_formula_or_cask(name, only:, method:, **options)
        ...
        Homebrew.with_no_api_env_if_needed(@without_api) do
          ...
          formula_or_kegs = if only != :cask
            begin
              case method
              when nil, :factory
                options = { warn:, force_bottle: @force_bottle, flags: @flags, prefer_stub: true }.compact
      ...
      def resolve_formula(name)
        options = { spec: @override_spec, force_bottle: @force_bottle, flags: @flags, prefer_stub: true }.compact
        Formulary.resolve(name, **options)
      end
